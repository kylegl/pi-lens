...
function checkSwitchExhaustiveness(content) {
    const issues = [];
...
function checkMissingReturns(content) {
    const issues = [];
...
function checkAnyTypeUsage(content) {
    const issues = [];
...
function formatTypeSafetyOutput(errors, warnings) {
    let output = "";
    if (errors.length > 0) {
