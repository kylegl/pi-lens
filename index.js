...
export default function (pi) {
    ...
    pi.registerCommand("lens-booboo-refactor", {
        description: "Interactive architectural refactor: picks worst offender by combined debt score, opens a browser interview with options + recommendation, then steers the agent with your decision. Usage: /lens-booboo-refactor [path]",
        handler: async (args, ctx) => {
            const targetPath = args.trim() || ctx.cwd || process.cwd();
            const fs = require("node:fs");
            ...
            const primaryRule = issues.length > 0
                ? [...issues].sort((a, b) => (ruleWeights[b.rule] ?? 0) - (ruleWeights[a.rule] ?? 0))[0].rule
                : null;
            const optionSets = {
                "large-class": [
                    {
                ...
                "long-method": [
                    {
                        ...
                        context: "Each logical step becomes a named helper — main method becomes orchestrator",
                        recommended: true,
                    },
                    {
                        value: "early-returns",
                ...
                "long-parameter-list": [
                    {
                        ...
                        context: "{ param1, param2, ... } — callers use named params, easy to extend",
                        recommended: true,
                    },
                    {
                        value: "builder",
                        label: "Use a builder pattern",
                        context: ".withX().withY().build() — good when many params are optional",
                    },
                    {
                        value: "reduce-params",
                ...
                "no-as-any": [
                    ...
                    },
                    {
                        ...
                        context: "Cast to unknown, narrow with a guard — safest for external data",
                    },
                    {
                        value: "suppress",
                        label: "Suppress with biome-ignore (justified boundary)",
                        context: "Only for third-party boundaries where types are genuinely unavailable",
                    },
                ],
                "no-non-null-assertion": [
                    {
                    ...
                    },
                    {
                        ...
                        context: "if (!x) return; — makes the assumption visible and catchable",
                    },
                ],
                "no-shadow": [
                    {
            ...
            const baseOptions = primaryRule && optionSets[primaryRule]
            ...
            const badges = [
                ...
                ...(metrics
                    ...
                    : []),
                ...[...ruleGroups.entries()].map(([r, n]) => `<span class="badge warn">${esc(r)} ×${n}</span>`),
            ].join("");
            const issuesHtml = issues
                .slice(0, 8)
                .map((i) => `<div class="issue"><span class="rule">${esc(i.rule)}</span> · L${i.line}${i.note ? ` · <span class="note">${esc(i.note)}</span>` : ""}</div>`)
                .join("");
...
document.addEventListener('keydown',e=>{if((e.ctrlKey||e.metaKey)&&e.key==='Enter')document.getElementById('f').submit();});
            ...
            let resolveChoice;
            const choicePromise = new Promise((res) => {
                resolveChoice = res;
            });
            ...
            const timeout = new Promise((res) => setTimeout(() => res(null), 10 * 60 * 1000));
            const userChoice = await Promise.race([choicePromise, timeout]);
            if (!userChoice) {
                server.close();
                ctx.ui.notify("⏰ Refactor session timed out — no response received.", "warning");
                return;
            }
            if (userChoice === "__skip__") {
                ctx.ui.notify("⏭️ Skipped — run /lens-booboo-refactor again to see the next offender.", "info");
                return;
            }
            // --- 9. Steer agent ---
