...
export class MetricsClient {
  ...
  private fileBaselines: Map<string, { content: string; entropy: number }> = new Map();
  ...
  getFileMetrics(filePath: string): FileMetrics | null {
    ...
    const preExistingLines = Math.max(0, baselineLines - Math.max(0, agentLines - (totalLines - baselineLines)));
   ...
   */
  getAICodeRatio(): { ratio: number; agentLines: number; preExistingLines: number; fileCount: number } {
    ...
    for (const [filePath, agentLines] of this.fileSessionWrites) {
      ...
      const baselineLines = baseline ? baseline.content.split("\n").length : totalLines;
   ...
   */
  getEntropyDeltas(): Array<{ file: string; start: number; current: number; delta: number }> {
    const results: Array<{ file: string; start: number; current: number; delta: number }> = [];
